...
void EnsureWalletIsBackedUp(const CChainParams& params)
{
    if (GetBoolArg("-walletrequirebackup", params.RequireWalletBackup()) && !pwalletMain->MnemonicVerified())
        throw JSONRPCError(
                ...
                "by using junocashd-wallet-tool first."
                );
}
...
UniValue listaddresses(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "regularly. If your wallet was created using junocashd version 4.7.0 \n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"junocashaddress\",     (string) The junocash address\n"
...
UniValue walletconfirmbackup(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "users. Please use the junocashd-wallet-tool utility (built or installed in the same directory\n"
            "as junocashd) instead. In particular, this method should not be used from junocash-cli, in order\n"
            ...
            "which can be obtained by making a call to z_exportwallet. The junocashd embedded wallet\n"
